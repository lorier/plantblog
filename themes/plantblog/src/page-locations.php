...
function pb_add_locations(){
    if( have_rows('garden_locations') ){
        while( have_rows('garden_locations')) : the_row();
            ...
            $image = (!empty($image)) ? $image : '<img src="'.get_site_url().'/wp-content/uploads/2016/07/Untitled-1.png"/>';

            $output = '<div class="locations wrap">';
            $output .='<div class="one-third first"><img class="location-image" src="'.$image.'"/></div>';
